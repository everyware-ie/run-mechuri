...
- §6 프리셋 3개(기본 경로 그림·불빛 러너·구간 점등) 전부 구현 (2026-08-31, 결과물 편집 화면과 함께). 구간 점등은 v0 단순화 있음 — [결과물 편집 노트](result-editing.md) 참고
...
- §7 러닝 데이터(거리·시간·페이스·심박) 구현. `drawStamps` — 마찬가지로 [결과물 편집 노트](result-editing.md)
  §7 참고. §7-3 "거리는 그려진 선 길이가 아니라 기록된 총 거리"를 그대로 지킴
- §8 합성 순서(배경 → 경로 → 러닝 데이터) 지킴
...
- **러닝 데이터 텍스트는 여전히 SVG 오버레이** — 로드된 JetBrains Mono를 또렷하게 쓰려고. Skia `Canvas` 위에
  절대 위치 `<Svg>`를 얹는다
...
- 러닝 데이터 텍스트 폰트: 미리보기(SVG, JetBrains Mono)와 최종 렌더러(Swift, 시스템 모노스페이스)가
