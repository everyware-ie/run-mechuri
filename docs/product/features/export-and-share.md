> 2026-09-13 후속: 백그라운드 실행·속도 개선의 현재 구조와 검증은 [인코딩 성능 노트](encoding-performance.md)에 둔다. 아래 2026-09-08 세대 번호 설명은 당시 구현 이력이며, 후속 구현은 작업별 취소 상태·직렬 큐·파일 이름별 진행 이벤트로 교체했다. 실패 알림의 `[다시 시도] [편집으로]`는 아래에서 검토한 방향이며 현재 코드는 `[확인]`으로 편집에 돌아가 다시 완료를 누르는 방식이다.
...
- `share.tsx`의 완성 화면을 "3안" 시안 S8b(공유 카드)에 맞췄다: 파일 경로 텍스트(개발용)를 빼고, 배경 위에 완주 시점 경로·러닝 데이터를 얹은 카드(`RouteThumbnail`, 300px) + 거리 + 러닝한 날을 둔다. 보관함 상세(`result/[id].tsx`)와 같은 구성 — "완성됐고 이게 보관함에 이렇게 남는다"가 바로 읽힌다
...
**FRD에 올렸다.** [경로 렌더링](../../specs/frd/route-rendering.md) §7-6 한 줄 문구, [결과물 편집](../../specs/frd/result-editing.md) §7 러닝 데이터와 문구가 정본이다.
...
- ~~**시안 S2 연결 화면 — 만들지 않는다**~~ → **정정 (2026-09-08).** 이미 만들어져 있었다(`connect.tsx`, 9/2~9/3).

  **권한을 묻는 화면이 아니다.** 첫 실행에만 뜨는 온보딩이고 "달린 길이 그림으로 남는다"를 보여준다. 권한은 목록을 열려 할 때 묻는다. **[공통 규칙](../../specs/frd/common-rules.md) §1-3과 부딪치지 않는다.** 이름만 보고 권한 화면으로 오해한 것이다.

  **다만 다른 곳과 어긋난다.** [홈과 보관함](../../specs/frd/home-and-library.md) §4-2가 **"첫 실행에만 나오고 사라지는 화면이면 그 역할을 못 한다"** 고 정해뒀다. 프리셋이 늘 때 알릴 자리로 써야 한다는 이유였는데, 구현은 정확히 첫 실행 전용이다.

  **W5 실사용 뒤에 판단한다.** 셋이 써보면 그 화면이 다시 필요한지가 드러난다(2026-09-08 결정)

  권한을 거부한 경우는 여전히 빈 상태(§2-1)가 감당하고, **설정에서 켜고 돌아오면 자동으로 다시 조회한다**
