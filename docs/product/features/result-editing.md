...
### 새 기록의 각인 기본값·최근 선택 (2026-09-13)

사용자 요청에 따라 [FRD §7 각인과 문구](../../specs/frd/result-editing.md)를 보완했다. 새 기록은 최근 직접 선택한 각인 프리셋으로 시작하고 선택 이력이 없으면 코너를 사용한다. `stamp-preference.ts`에 프리셋 ID만 별도 저장한다. 저장값 읽기는 새 기록을 시작할 때 기다리며, 기존 초안·결과물을 불러올 때는 적용하거나 덮어쓰지 않는다. 빠른 연속 선택은 저장 순서를 보장하고 읽기 도중 선택한 값이 있으면 최신 선택을 우선한다. 기존 저장분의 `IDENTITY_STAMP` 간결 폴백은 그대로 두어 옛 결과물 모양을 바꾸지 않는다. 최초 코너·재시작 후 최근 선택·연속 저장 순서·읽기 중 새 선택·저장 오류 폴백을 저장소 mock으로 확인했고 타입 검사와 관련 파일 lint를 통과했다. `edit.tsx`는 기존 refs/immutability 오류 16개가 동일하게 남아 있으며 새 오류는 없다.

### 하단 이전 버튼 인지성 (2026-09-13)

내부 QA에서 ‘배경 바꾸기’와 이전 버튼이 잘 보이지 않는다는 사용자 요청을 반영했다. [FRD §1 화면 구조](../../specs/frd/result-editing.md)에 표현 규칙을 보완하고, `edit.tsx`의 공통 `stepPrevious`에 기존 테마의 `borderStrong` 테두리·밝은 본문색·왼쪽 화살표·눌림 배경을 적용했다. 각인 단계의 표기는 ‘이전 · 드로잉’으로 목적지를 명확히 했다. 높이와 하단 배치는 유지하며, 배경 재선택 및 단계 전환 핸들러는 그대로 사용한다.

### 각인 프리셋 노출 순서 (2026-09-13)

사용자가 내부 QA 결과에 따라 코너·글래스·레일을 먼저 노출하고 나머지는 예상 선호도로 재배열하도록 요청했다. [FRD §7 각인과 문구](../../specs/frd/result-editing.md)에 순서와 가설을 기록했다. `STAMP_LAYOUTS` 배열만 재정렬해 코너 → 글래스 → 레일 → 스택 → 스탯바 → 원라인 → 간결 순으로 표시한다. 스택의 큰 거리 숫자, 스탯바의 통계 가독성을 먼저 보여주고 원라인·간결을 뒤에 둔다. 선택은 배열 인덱스가 아닌 프리셋 ID로 처리하므로 기존 선택과 저장 결과는 유지되며, 당시에는 신규 기본값도 유지했다. 이후 새 기록의 코너·최근 선택 정책은 위 후속 항목에서 별도로 적용한다.
