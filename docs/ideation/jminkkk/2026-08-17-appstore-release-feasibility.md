...
#### 먼저 — iOS는 번호가 두 개다 (버전 ≠ 빌드)

아래 원문의 *"같은 버전의 이후 빌드"* 를 읽으려면 이 구분이 먼저다. **팀에 iOS 경험자가 없으므로 여기부터 적는다.**

| Info.plist 필드 | 통칭 | 예시 | 성격 |
|---|---|---|---|
| `CFBundleShortVersionString` | **버전** (마케팅 버전) | `1.0` | **사용자에게 보이는 번호.** App Store에 표시된다 |
| `CFBundleVersion` | **빌드** | `1`, `2`, `3`… | 같은 버전 안의 일련번호. **업로드할 때마다 올린다** |

TestFlight에는 `1.0 (3)` 처럼 **버전 (빌드)** 형태로 표시된다.

```
1.0 (1)   ← 첫 업로드. 외부 테스터에게 배포하려면 Beta App Review 필수
1.0 (2)   ← 코드 수정 후 재업로드. "심사가 없을 수도 있다"
1.0 (3)   ← 또 수정. 마찬가지
1.0 (7)   ← 같은 버전 안이면 계속 마찬가지
──────────
1.1 (1)   ← 버전을 올렸다 → 새 버전이므로 다시 전체 심사
```

**즉 "코드를 고치면 심사를 다시 받나?"의 답은 이렇다.** 코드를 고치면 무조건 새 *빌드*지만,
**버전 번호를 그대로 두고 빌드 번호만 올리면 베타 심사를 건너뛸 수 있다.** 버전을 올리면 다시 심사다.

**함의(개인 판단). 9월 내내 버전을 `1.0`으로 고정하고 빌드만 올리는 것이 이 역산의 숨은 전제다.**
그래야 W4~W5에 매일 빌드를 밀어넣으며 실사용할 수 있다. 버전을 올리는 시점은 정식 출시 때뿐이다.
**중간에 버전을 올려야 할 일이 생기면 그 순간 전체 심사가 다시 붙는다** — 예를 들어 9월 중
HealthKit 권한을 추가하기로 하면 사용 목적 문구가 바뀌고, 재심사 가능성이 올라간다. `[확인 필요]`
...
|---|---|
| 첫 빌드 (`1.0 (1)`) | **전체 심사 필수** |
| 같은 버전의 이후 빌드 (`1.0 (2)`, `1.0 (3)`…) | **심사가 없을 수도 있다** (`might not` — 보장 아님) |
| 다른 버전 (`1.1 (1)`) | 원문에 없음. **새 버전이므로 심사를 전제해야 한다** |
...
- 리젝 대응(수정+재제출) = 2~3일
- 9/22(화) 당일이 아니라 **9/18(금)** 을 승인 목표로 둔다 — 주말 버퍼
  (9/19는 토요일이다. 주말 버퍼를 두려면 승인 목표는 금요일인 9/18이어야 한다)
- **W5 실사용에 TestFlight가 필요**하고, **첫 빌드는 전체 심사를 받아야 한다**(§4-2)
- 첫 베타 심사에 낼 빌드의 하한은 **W2 종료(8/25) "코어 루프 관통"** 직후

### 5-2. 역산 결과 — 첫 베타 심사를 앞으로 당긴다

| 주차 | 기간 | 회의록이 정한 그 주 산출물 | 날짜 | 이 트랙의 마일스톤 |
|---|---|---|---|---|
| **W1** | 8/12~8/18 | 1주차 분담 조사<br>(jminkkk: 기술 스택 + 9/22 출시 방식) | **8/18 (화)** | **Apple Developer Program 등록 착수**<br>담당: jminkkk |
| **W2** | 8/19~8/25 | **코어 루프 관통**<br>데이터 → 결과물 1편 | 8/19~8/21 | 등록 승인 대기 · 승인 직후 §3-3 신속 심사 조건 확인 |
| **W2** | 8/19~8/25 | 〃 | **8/25 (화)** | 코어 루프 관통 = **첫 심사에 낼 최소 기능 확보**<br>⚠️ 여기가 밀리면 아래가 전부 밀린다 |
| **W3** | 8/26~9/1 | `TODO: 회의록 확인` | **~9/1 (화)** | **TestFlight 첫 빌드 `1.0 (1)` 제출 → 베타 심사 통과**<br>이 트랙의 핵심 제안 |
| **W4** | 9/2~9/8 | **기능 동결** | 9/2~9/8 | 개발 계속. **버전은 `1.0` 고정, 빌드만 올린다**(§4-2)<br>심사가 다시 걸릴 가능성은 남는다 |
| **W4** | 9/2~9/8 | 〃 | **9/8 (화)** | 기능 동결 (회의록 원안 유지) |
| **W5** | 9/9~9/15 | **실사용**<br>3명이 실제로 뛰고 올려보기 | 9/9~9/14 | TestFlight로 실사용. 빌드 갱신 계속 |
| **W6** | 9/16~9/22 | **출시** | **9/15 (화)** | **App Store 정식 제출** — 전체 심사(베타 통과와 무관) |
| **W6** | 9/16~9/22 | 〃 | 9/17~**9/18 (금)** | 정식 심사 통과 목표. **리젝 시 이 구간에서 1회 대응**<br>주말 버퍼로 금요일을 목표로 둔다 |
| **W6** | 9/16~9/22 | 〃 | **9/22 (화)** | **출시** |

> **요일 표기는 2026년 달력으로 재확인했다.** 8/18·8/25·9/1·9/8·9/15·9/22는 **전부 화요일**이고,
> 9/18이 금요일, 9/19는 토요일이다.
>
> `TODO:` **W3 산출물이 회의록에서 확인되지 않았다.** [08-11 회의록](https://github.com/everyware-ie/mechuri-docs/blob/main/meetings/2026-08-11/synthesis.md)을 열어 채울 것.
> W3에 TestFlight 첫 제출을 얹자는 것이 이 트랙의 제안이므로, **그 주에 원래 무엇이 잡혀 있었는지가
> 제안의 실현가능성을 좌우한다.**
